...
* 良好な応答時間
* バックグラウンドジョブの高いスループット
* プロセス飢餓(fdなどのシステム資源を獲得できない状態の回避
...
* テキストエディタとコンパイラ
* ToDo: シーケンス図っぽく
* 入力 -> 割り込み発生 -> テキストエディタのプロセスを起床 -> TIF_NEED_RESCHED フラグを設定(スケジューラを強制的に起動) -> スケジューラがテキストエディタへプロセス切り替え -> 文字を画面に表示 -> 待ち -> コンパイラのプロセスが実行
...
<img src="./pictures/table-7-3-2.png" height="100%" width="100%"/>
...
<img src="./pictures/numa.png" height="70%" width="70%"/>
...
* I/Oバウンド型かCPUバウンド型に自動判別している
* リアルタイムかどうかはどうやって自動判別？
...
* 優先度が高い => I/Oバウンド型になりやすい
* I/Oバウンド型になると、クォンタムを使い切っても(通常)活動状態のまま
